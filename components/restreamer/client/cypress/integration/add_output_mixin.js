describe("ADD MIXIN OTPUT", () => {
	...
	it("Set rtmp://", () => {
		...
		cy.get('[placeholder="rtmp://..."]').invoke('val', urlToPaste).trigger('input')
	});
	
	it("Set preview", () => {
		const previewToPaste = 'https://www.youtube.com/watch?v=99567P5FiD0'
		cy.get('[placeholder="optional preview url"]').invoke('val', previewToPaste).trigger('input')
	});
	...
	it("Set teamspeak", () => {
		...
		cy.get('[placeholder="ts://<teamspeak-host>:<port>/<channel>?name=<name>"]').invoke('val', teamspeakToPaste).trigger('input')
	});
	...
	it("Assert", () => {
		...
		cy.get("span:contains('/Translation/Music')").should('have.text', 'ts://allatra.ruvoice.com:10335/Translation/Music');
	});
});